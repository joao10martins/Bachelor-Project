...
Blob::Blob(std::vector<cv::Point> _contour) {
	...
	cv::Point currentLeftCorner;

	currentCenter.x = (currentBoundingRect.x + currentBoundingRect.x + currentBoundingRect.width) / 2;
	currentCenter.y = (currentBoundingRect.y + currentBoundingRect.y + currentBoundingRect.height) / 2;

	currentLeftCorner.x = (currentBoundingRect.x + currentBoundingRect.width) - 20;
	currentLeftCorner.y = (currentBoundingRect.y + currentBoundingRect.height);

	centerPositions.push_back(currentCenter);
	leftCorner.push_back(currentLeftCorner);
	...
	dblWidth = (float)currentBoundingRect.width;
	dblHeight = (float)currentBoundingRect.height;
